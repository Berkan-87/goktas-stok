...
const MaterialDepo = () => {
  ...
  const tabs = [
    { id: 'mdf', label: '📋 MDF\'ler', icon: '📋' },
    { id: 'glue', label: '🧴 Tutkallar', icon: '🧴' },
    { id: 'edgeband', label: '📐 Kenar Bantlar', icon: '📐' },
    { id: 'pvc', label: '🔲 PVC\'ler', icon: '🔲' }
  ];
  ...
  const renderMDFForm = () => (
    <>
      <input
        type="text"
        placeholder="Malzeme Adı (örn: 18mm MDF)"
  ...
  // Render Tutkal Formu
  const renderGlueForm = () => (
    <>
      <input
        type="text"
        placeholder="Malzeme Adı (örn: İskelet Tutkalı)"
        value={newMaterial.name}
        onChange={(e) => setNewMaterial({ ...newMaterial, name: e.target.value })}
        className="input-field"
        required
  ...
  // Render Kenar Bant / PVC Formu
  const renderColorForm = () => (
    <>
      <input
        type="text"
        placeholder="Malzeme Adı (örn: Beyaz Kenar Bant)"
        value={newMaterial.name}
        onChange={(e) => setNewMaterial({ ...newMaterial, name: e.target.value })}
        className="input-field"
        required
  ...
  const renderAddForm = () => {
    switch (activeTab) {
      case 'mdf': return renderMDFForm();
      case 'glue': return renderGlueForm();
      case 'edgeband':
      case 'pvc': return renderColorForm();
      default: return null;
    }
  };
    ...
    return (
      <div key={material._id} className="bg-white rounded-lg p-4 border border-gray-200 hover:shadow-md transition-shadow">
        ...
        <div className="mt-3 flex items-center justify-between">
          ...
          <div className="flex gap-1">
            ...
            <button
              ...
              className={`p-1.5 rounded ${material.stock <= 0 ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
              title="Stok Çıkar"
        ...
        <div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden mt-2">
          <div 
            className={`h-full rounded-full transition-all ${material.stock <= material.criticalLevel ? 'bg-red-500' : material.stock <= material.criticalLevel * 2 ? 'bg-yellow-500' : 'bg-green-500'}`}
